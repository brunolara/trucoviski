...
Status: **F6b no ar — 55,3% vs v2** (era 49,7%). Metade do caminho a 60%. Eval
rápida: `pnpm sweep:v3 -- --vs-v2-only` (só arena vs v2). Próximo salto provável
exige F10 (informação na aposta) — calibração de limiar saturando.
...
| Métrica                                | Alvo      | Hoje                           |
| -------------------------------------- | --------- | ------------------------------ |
| Winrate vs v2                          | **≥ 60%** | **55,26%** (F6b, N=30k test)   |
| Winrate vs v1 (não regredir)           | ≥ 55,2%   | a re-medir pós-F6b             |
| Self-play: partidas fechadas em mão ≥9 | < 31,7%   | a re-medir (rgMax=12 sobe 12s) |
...
| --------------------- | ------------ | ------------------------------------------------------------------------------ |
| **v3 vs v2 (F6b)**    | **55,26%**   | `pnpm arena --a heuristic-v3 --b heuristic-v2 --games 15000 --seed-block test` |
| v3 vs v2 (train)      | 55,26%       | `--games 5000 --seed-block train`                                              |
| v2 vs v1 (N=40k)      | **55,21%**   | `--a heuristic-v2 --b heuristic-v1 --games 20000`                              |
| **oracle vs v2 (F7)** | **99,70%**   | `--a oracle --b heuristic-v2 --games 2000`                                     |

### F6b — push vs v2 (2026-07-26, só vs v2)

Iteração rápida pedida pelo humano: medir **só vs v2**. Achados:

| Preset                                                  | vs v2     |
| ------------------------------------------------------- | --------- |
| F5 à mão                                                | 49,7%     |
| Vencedor F6 (offsets 4.5/2.25, rgMax=9)                 | 52,1%     |
| **F6b: propose=-1.5, response=5, rgMax=12, distanceOn** | **55,3%** |

Exploits “knownWin / lost1” testados e **rejeitados** (−5 a −10pp).
`pnpm sweep:v3 -- --vs-v2-only` pula vsV1/self (~3× mais rápido por eval).

Teto de calibração de limiar parece ~55–56% sem informação nova. Gap a 60% ainda
existe (oracle 99,7%); F10 é o próximo salto real.

### Resultados F6 / F7 / F8 / F9 (2026-07-26)

#### F6 — recalibrar — **PORTÃO FAIL**

Sweep completo (532s wall, pool=10, climb paralelo): vencedor test `vsV2=51,53%`
`vsV1=54,82%` `self≥9=24,2%`. Portão 53,5 / 55,2 / 31,7 → **FAIL / FAIL / OK**.
Confirm #3 chegou a 54,39% vs v2 mas morreu no vs v1 (54,08%). Artefato em
`docs/v3-sweep-result.json`.

**Não promoveu** `V3_FEATURES` (regra 6 + não regredir vs v1). Não re-varreu
(regra 8). → F7.

Bug corrigido no caminho: `fitness: -Infinity` virava `null` no IPC JSON dos
workers — trocado por `-1`.

#### F7 — teto — **99,70% vs v2** (N=4k, seed test)

| oracle vs v2 | Leitura                 | Ação                             |
| ------------ | ----------------------- | -------------------------------- |
| **99,70%**   | >> 65%: há folga enorme | **Seguir.** O gap é de política. |

Sanidade: oracle vs oracle = 50,0% (N=200). O 99,7% é exploit real dos buracos
do v2 (Anexo A) com informação perfeita nas decisões de aposta — teto **fraco**
ainda assim responde a pergunta.

Infra: `PlayerView.allHands` + `createMatch(..., { revealAllHands })` + política
`oracle` na arena. Produção nunca liga o flag.

#### F8 — atribuição — **o dinheiro é o truco**

Split: turno `isCardOnly` → política de carta; senão → política de truco (decide
o turno inteiro, inclusive jogar carta sem apostar).

| Política                     | vs v2 (N=4k)      |
| ---------------------------- | ----------------- |
| v3 puro                      | **49,73%**        |
| v3 cartas + **oracle truco** | **99,70%**        |
| **oracle cartas** + v3 truco | **49,73%** (= v3) |
| oracle puro                  | **99,70%**        |

Leitura: turnos com aposta no menu dominam o placar. Carta perfeita nos residual
turns (sem truco legal) vale ~0pp. **Congelar jogo de cartas; investir em
truco.** (O “oracle cartas + v3 truco” ≈ v3 porque truco quase sempre está legal
— o oráculo quase nunca age nesse híbrido.)

#### F9 — busca no lado do truco

| Passo                                                    | Resultado                                              | Decisão                                                            |
| -------------------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------ |
| Híbrido `v3-cards-mc-truco` (MC só quando truco no menu) | triagem 52,70% (N=2k); confirm **51,82%** vs v2 (N=5k) | +2pp vs v3 no v2                                                   |
| Mesmo híbrido vs v1                                      | **40,24%** (N=5k)                                      | **KILL** — regrediu 15pp vs o v3 (55,93%). Exploit do v2 (regra 7) |
| Self-play do híbrido                                     | **99,7% fecham em 12** (N=2k)                          | **KILL** — pior que o v2 (38,6%)                                   |
| F9.4 truco no rollout                                    | 51,25% (N=2k)                                          | **Revertido** — não pagou                                          |
| F9.2 rollout = v3                                        | **45,65%** (N=2k)                                      | **Revertido** — piorou                                             |

MC puro remediado: 52,3% (N=1k) — melhor que o 47,7% antigo do doc, ainda longe
de 60% e ainda viciado em fechar a 12 (~71%).

**Portão do híbrido: FAIL.** Não promover. O MC de truco com info imperfeita
compra ~2pp contra o v2 e **sangra** contra o v1.

**Próximo (F10):** modelo de adversário. F8 + F7 dizem que o salto a 60%+ exige
informação (o oráculo a 99,7% é “sei as cartas na hora de apostar”), não mais
limiar nem PIMC cego. F10 é o último recurso do plano e o mais perigoso (overfit
ao v2) — portão triplo sem exceção.
---
